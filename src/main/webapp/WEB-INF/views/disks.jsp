...
<html>
    <head>
        <title>All users / disks - dvdshare</title>
        <meta charset="UTF-8">
    ...
    <body>
        <div class="container">
            ...
            <ul id="tabs" class="nav nav-tabs">                
                <li id="tab-allusers" role="presentation"><a href="<c:url value='/users'/>">All users</a></li>                
                <li id="tab-alldisks" role="presentation" class="active"><a href="<c:url value='/users/disks'/>">All disks</a></li>                
            </ul>
                    ...
                    <c:forEach var="disk" items="${disks}">
                        <c:if test="${disk.id != null}">                           
                            <tr>
                                ...
                                <td class="request">
                                    <c:if test="${disk.owner == disk.holder}">                                        
                                        <c:choose>                                            
                                            <c:when test="${f:colcontains(requests, disk.request) && !f:colcontains(principalRequests, disk.request)}">
                                                <span id="btn-request-dis" class="glyphicon glyphicon-ok btn-disabled"></span>    
                                                <span id="btn-cancel-dis" class="glyphicon glyphicon-remove btn-disabled"></span>
                                            </c:when>
